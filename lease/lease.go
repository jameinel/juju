...
func (m *leaseManager) workerLoop(stop <-chan struct{}) error {
	...
	for {
		...
		case release := <-m.releaseLease:
			...
			err = m.leasePersistor.RemoveToken(release.Token.Namespace)
			if err != nil {
				release.Response <- err
				continue
			}
			notifyOfRelease(releaseSubs[release.Token.Namespace], release.Token.Namespace)
			release.Response <- nil
