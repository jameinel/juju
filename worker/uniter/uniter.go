...
func (u *Uniter) init(unitTag names.UnitTag) (err error) {
	...
	u.contextFactory, err = context.NewFactory(
		u.paths, u.st, unitTag, u.getRelationInfos,
	)
	if err != nil {
		return err
	}
...
func (u *Uniter) runAction(hi hook.Info) (err error) {
	...
	if err != nil {
		// This should really *really* never happen, but let's not panic.
		return errors.Annotatef(err, "action % not an action", hi.ActionId)
	}
