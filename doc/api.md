...
This document is intended to describe how to connect to the Juju API and make
requests to the Juju Controller. 

## Websocket Connection

By default, Juju serves an HTTPS endpoint on port 17070. All of the TLS
certificates that are generated by Juju will be signed by the controller's
certificate authority (`ca-cert`). Clients can connect to that endpoint and
issue an HTTP UPGRADE request to initialize the conversation. The actual
conversation takes the form of asynchronous JSON formatted Request and
Responses. Each requests gets a unique integer identifier from the client,
which the server will process and return a Response using the same identifier.

Every controller has its own UUID, and every model within that controller also
...
TODO: Login is different when using Macaroons, as you supply your login
credentials as part of the HTTP headers, and pass Login() itself an empty
Username.
