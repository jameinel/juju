...
func NewListImageMetadataCommand() cmd.Command {
	return modelcmd.Wrap(&listImageMetadataCommand{})
}

const listCommandDoc = `
List information about image metadata stored for a Juju model.
This list can be filtered using various filters as described below.

More than one filter can be specified, the result will contain metadata that matches all filters in combination.
...
// listImageMetadataCommand returns stored image metadata.
type listImageMetadataCommand struct {
...
func (c *listImageMetadataCommand) Init(args []string) (err error) {
	if len(c.Series) > 0 {
		result := []string{}
...
func (c *listImageMetadataCommand) Info() *cmd.Info {
	return &cmd.Info{
		Name:    "list-image-metadata",
...
func (c *listImageMetadataCommand) SetFlags(f *gnuflag.FlagSet) {
...
func (c *listImageMetadataCommand) Run(ctx *cmd.Context) (err error) {
	api, err := c.getImageMetadataListAPI()
	if err != nil {
		return err
	}
	defer api.Close()
...
func (c *listImageMetadataCommand) getImageMetadataListAPI() (MetadataListAPI, error) {
	if c.api != nil {
		return c.api, nil
	}
